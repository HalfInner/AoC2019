...
class RouterTracker:
    ...
    def __move_operation(self, operation):
        if operation is RouterTracker.TURN_RIGHT:
            self.__direction += 1
        elif operation is RouterTracker.TURN_LEFT:
            self.__direction += -1
        else:
            raise Exception('Not supported operation Op({})'.format(operation))
        self.__direction %= 4
        {
